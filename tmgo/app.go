...
ByteBuffer call_fn_ptr_with_bytes(void* abci_ptr, ByteBuffer bytes, int32_t index) {
...
func NewABCFApplication(abci_ptr unsafe.Pointer, index int) *ABCFApplication {
	return &ABCFApplication{abci_ptr, index}
}

func call_abci(abci_ptr unsafe.Pointer, req *abcitypes.Request, index int) abcitypes.Response {
	data, _ := req.Marshal()
	...
	bb := C.call_fn_ptr_with_bytes(abci_ptr, arg, C.int32_t(index))
...
func (a ABCFApplication) Info(req abcitypes.RequestInfo) abcitypes.ResponseInfo {
	...
	abci_resp := call_abci(a.abci_ptr, abci_req, a.index)
