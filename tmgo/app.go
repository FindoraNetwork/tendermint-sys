...
typedef ByteBuffer (*bytes_func_ptr)(ByteBuffer);

ByteBuffer call_fn_ptr_with_bytes(void* abci_ptr, ByteBuffer bytes) {
...
func NewABCFApplication(abci_ptr unsafe.Pointer) *ABCFApplication {
    return &ABCFApplication{abci_ptr}
}

func call_abci(abci_ptr unsafe.Pointer,req *abcitypes.Request) abcitypes.Response {
    data, _ := req.Marshal()
    ...
    bb := C.call_fn_ptr_with_bytes(abci_ptr, arg)
...
func (a ABCFApplication) Info(req abcitypes.RequestInfo) abcitypes.ResponseInfo {
    ...
    abci_resp := call_abci(a.abci_ptr, abci_req)
